...
import minicraft.screen.TexturePackMenu;
...
public class Game {
	...
	public static String textureDir = systemGameDir + localGameDir + "" + "/texturepacks"; // The directory where Texture Packs are stored
	public static String modsDir = systemGameDir + localGameDir + "" + "/mods";
	...
	private static void init() {
		...
		try {
			if (TexturePackMenu.getTexturePack().equalsIgnoreCase("default")) {
				...
				System.out.println(TexturePackMenu.getTexturePack());
				screen = new Screen(new SpriteSheet(ImageIO.read(
						new File(TexturePackMenu.getTexturePack())
						)));
				lightScreen = new Screen(new SpriteSheet(ImageIO.read(new File(TexturePackMenu.getTexturePack()))));
			}
		} catch (IOException e) {
			JOptionPane.showMessageDialog(null, "A texture pack was removed while it was in use. Please restart to resolve this error.");
			TexturePackMenu.resetTexturePack();
	...
	public static void initWorld() { // this is a full reset; everything.
		...
		scoreTime = (Integer) Settings.get("scoretime") * 60 * 60;
	...
	private static void renderGui() {
		...
		if (Game.isMode("speedrun")) {
			Font.draw("Time: " + InfoDisplay.getTimeString(), screen, Screen.w/2-9*8, 2, Color.RED);
		}
		...
		if (!Game.isMode("creative")) {
			for (int i = 0; i < 10; i++) {
				...
				if ( (Settings.get("ui") + "").equalsIgnoreCase("default") ) {
					// renders armor
					...
					// renders your current red hearts, or black hearts for damaged health.
					color = (i < player.health) ? Color.get(-1, 200, 500, 533) : Color.get(-1, 100, 000, 000);
					screen.render(i * 8, 0, 0 + 12 * 32, color, 0);

					if (player.staminaRechargeDelay > 0) {
						// creates the white/gray blinking effect when you run out of stamina.
						color = (player.staminaRechargeDelay / 4 % 2 == 0) ? Color.get(-1, 555, 000, 000) : Color.get(-1, 110, 000, 000);
						screen.render(i * 8, 8, 1 + 12 * 32, color, 0);
					} else {
						// renders your current stamina, and uncharged gray stamina.
						color = (i < player.stamina) ? Color.get(-1, 220, 550, 553) : Color.get(-1, 110, 000, 000);
						screen.render(i * 8, 8, 1 + 12 * 32, color, 0);
					...
					screen.render(i * 8 + (Screen.w - 80), 2, 2 + 12 * 32, color, 0);
				}
			}
		}
