...
public class LevelGen {
	...
	protected static byte[][] createAndValidateMap(int w, int h, int level) {
		...
		if (level == -5)
			return createAndValidateRoom(w, h);
  ...
  public static byte[][] createAndValidateRoom(int w, int h) {
		random.setSeed(worldSeed);
		int attempt = 0;
		do {
			byte[][] result = createDungeon(w, h);
			...
			if (count[Tiles.get("Cloud").id & 0xff] < 100) continue;
			if (count[Tiles.get("Cloud Wall").id & 0xff] < 100) continue;
	...
	private static byte[][] createRoom(int w, int h) {
		LevelGen noise1 = new LevelGen(w, h, 8);
		LevelGen noise2 = new LevelGen(w, h, 8);
		...
		for (int i = 0; i < w * h / 450; i++) {
			...
			for (int yy = y - 1; yy <= y + 1; yy++)
				for (int xx = x - 1; xx <= x + 1; xx++) {
					if (map[xx + yy * w] != Tiles.get("Cloud Wall").id) continue lavaLoop;
				}
