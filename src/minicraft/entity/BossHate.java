...
public class BossHate extends EnemyMob implements Boss {
	private static MobSprite[][] sprites = MobSprite.compileMobSpriteAnimations(0, 14);
	private static int[] lvlcols = {
		Color.get(-1, Color.rgb(100, 0, 0), Color.rgb(106, 255, 120), Color.rgb(255, 0, 0)),
	};
	 ...
	 */
	public BossHate(int lvl) {
		super(lvl, sprites, lvlcols, 2000, 100);
	}
	...
	public void tick() {
		...
		if (random.nextInt(500)==1) {
			getLevel().add(new Skeleton(0), x, y + 5);
			getLevel().add(new Skeleton(0), x, y - 5);
			getLevel().add(new Skeleton(0), x + 5, y);
			getLevel().add(new Skeleton(0), x - 5, y);
		}
