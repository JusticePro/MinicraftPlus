...
import minicraft.screen.CheatMenu;
	...
	public void tick() {
		...
		if (Game.getMenu() == null && !Bed.inBed) {
			...
			if ((xa != 0 || ya != 0) && (staminaRechargeDelay % 2 == 0 || isSwimming()) && !Game.saving) {
				double defaultSPD = moveSpeed * (potioneffects.containsKey(PotionType.Speed) ? 1.5D : 1);
				
				double spd = defaultSPD;
				
				if (CheatMenu.speed.getValue().equalsIgnoreCase("Double")) {
					spd = defaultSPD * 2;
				}
				
				if (CheatMenu.speed.getValue().equalsIgnoreCase("Triple")) {
					spd = defaultSPD * 3;
				}
				
				if (CheatMenu.speed.getValue().equalsIgnoreCase("Quadruple")) {
					spd = defaultSPD * 4;
				}
				
				if (CheatMenu.speed.getValue().equalsIgnoreCase("Quintuple")) {
					spd = defaultSPD * 5;
				}
