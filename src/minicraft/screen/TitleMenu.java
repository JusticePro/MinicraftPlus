...
//				new SelectEntry("Join Online World", () -> Game.setMenu(new MultiplayerMenu())),
				new SelectEntry("Multiplayer", () -> {
					...
					Menu meh = new Menu.Builder(true, 5, RelPos.CENTER,
							new SelectEntry("Multiplayer will return!", null)
							).createMenu();
					
					Game.setMenu(new Display(meh));
				...
				displayFactory("More", 
						...
						new BlankEntry(),
						new SelectEntry("Discord", () -> {
							try {
								Desktop.getDesktop().browse(new URI("https://discord.me/page/minicraft"));
	...
	public void render(Screen screen) {
		...
		if (notchBirthday()) {
			Font.drawCentered("Happy birthday Notch!", screen, 60, Color.CYAN);
		}else {
	...
	private boolean notchBirthday() {
		...
		if (date.getMonth().equals(Month.JUNE) && date.getDayOfMonth()==1) {
			return true;
		}
		return false;
	}
